...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.75\linewidth}
        \centering
        \includegraphics[width=\textwidth]{res/knn_vs_mc_spatial.pdf}
    \end{subfigure}
    \begin{subfigure}[b]{0.75\linewidth}
        \centering
        \includegraphics[width=\textwidth]{res/knn_spatial_analysis.pdf}
    \end{subfigure}
...
The KD-Tree Accelerated Non-Local Means algorithm is an interesting approach to improve the efficiency of the NLM algorithm using the idea from Monte Carlo NLM and improving on it. It does not provide a significant improvement, especially considering the bottlenecks in parallelization which can be a problem for large images, but it's still a valid approach that can be explored further. The paper by Adams et al.~\cite{adams2009gaussian} provides a more advanced approach that can be implemented for even better performance, adding new techniques to improve the speed of the algorithm and accuracy. It involves \emph{Monte Carlo sampling} and \emph{Splat-blur-slice} for faster search. It also solves issues such as the curse of dimensionality by using Principal Component Analysis (PCA) to reduce the dimensionality of the patches before inserting them into the KD-Tree. While this method was also tried in our implementation, it did not provide a significant improvement in performance or image quality.